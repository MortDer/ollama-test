<template>
  <fieldset class="z-date-time-picker fieldset">
    <div
      :class="[{ 'disabled-text': !enabled }, 'z-date-time-picker__container']"
    >
            ...
            <div class="z-date-time-picker__list">
              <div class="z-date-time-picker-list">
                <q-input
                  ...
                  dark
                  :disable="
                    dateRadio !== DateRadioOptions.OtherPeriod || !enabled
                  "
                  :class="[
                    'non-selectable',
                    { otherPeriod: isOtherPeriod },
                    'z-date-time-picker-list__item',
                  ]"
                ...
                <q-input
                  ...
                  :class="[
                    'non-selectable',
                    { otherPeriod: isOtherPeriod },
                    'z-date-time-picker-list__item z-date-time-picker-list__item--small',
                  ]"
                  @update:model-value="setDates"
                />
              </div>
...
const { formatDate, formatDateToIso } = useFormatters();
const dateRadio = ref<DateRadioOptions>(
  props.period ? props.period : DateRadioOptions.ExpiredFull,
);
...
const radioPaddingLeftComputed = computed(() =>
  props.radioPaddingLeft ? props.radioPaddingLeft : 4,
);
const isOtherPeriod = computed(
  () => dateRadio.value === DateRadioOptions.OtherPeriod,
);
  ...
  (value) => {
    ...
    switch (value) {
      ...
      case DateRadioOptions.OtherPeriod:
        if (
          (props.allSchedule || props.unlimited) &&
...
function formatDatesForInput() {
  startDateFormatted.value = startTime.value
    ? formatDate(startDate.value, "YYYY-MM-DD")
    : "";
  endDateFormatted.value = endTime.value
...
function setDates() {
  ...
  emit(
    "update:start-date",
    startDateFormatted.value && startTime.value
      ? formatDateToIso(dayjs(startDateExport).toDate())
      : "",
  );
  emit(
    "update:end-date",
    endDateFormatted.value && endTime.value
      ? formatDateToIso(dayjs(endDateExport).toDate())
      : "",
  );
...
function setOnGoingDates() {
  ...
  emit(
    "update:start-date",
    startTime.value ? formatDateToIso(dayjs(startDateExport).toDate()) : "",
  );
  emit(
    "update:end-date",
    endTime.value ? formatDateToIso(dayjs(endDateExport).toDate()) : "",
  );
  emit("update:period", dateRadio.value);
}
