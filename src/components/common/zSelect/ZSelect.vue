...
const computedNoOptionsText = computed(() => props.noOptionsText || t('noData.label'));
...
const opts = ref(getOptions(props.optionsList, props.optionsKey, props.options));
...
function filterFn(
    ...
    update(() => {
        const needle = val.toString()
            .toLowerCase();
        const options = getOptions(props.optionsList, props.optionsKey, props.options) || [];
